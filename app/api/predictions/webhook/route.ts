...
async function handleWebhook(request: Request) {
  try {
    ...
    if (paymentId) {
      const verified = await verifyPaymentByIdInternal({ paymentId, orderId: searchOrderId, product: 'predictions' })
      if (verified?.verified) {
        console.log('[WEBHOOK] ✅ Verified through helper for order:', searchOrderId)
