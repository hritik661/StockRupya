...
export async function POST(req: Request) {
  try {
    ...
    // Revert top-gainer payment only
    if (useDatabase && sql) {
      try {
        ...
        try {
          await sql`UPDATE payment_orders SET status = 'reverted' WHERE user_id = ${user.id} AND product_type = 'top_gainers'`
        } catch (e) {
          console.warn('[REVERT-PAYMENT] Failed to update payment_orders status (continuing):', e)
        }

        console.log('🔄 [REVERT-PAYMENT] Reverted payment for user (top_gainers + predictions):', user.id)

        // Return full user data from DB (so client does not overwrite balance/other fields)
        try {
          ...
          return NextResponse.json({
            success: true,
            message: 'Top gainer payment access has been reverted.',
            user: {
              id: u?.id || user.id,
          ...
          return NextResponse.json({
            success: true,
            message: 'Top gainer payment access reverted (partial).',
            user: { id: user.id, email: user.email, isTopGainerPaid: false, isPredictionPaid: user.is_prediction_paid === true }
          })
        }
      } catch (err) {
