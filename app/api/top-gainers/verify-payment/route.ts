  ...
  try {
    ...
    if (!orderId) {
      if (paymentIdFromQuery) {
        const result = await verifyPaymentByIdInternal({ paymentId: paymentIdFromQuery, orderId: null, product: "top_gainers" })
        if (result?.verified) {
    ...
    if (status !== "paid" && paymentIdFromQuery) {
      const verifiedByPaymentId = await verifyPaymentByIdInternal({
        ...
        orderId,
        product: productType === "predictions" ? "predictions" : "top_gainers",
      })
    ...
    if (finalStatus !== "paid") {
      ...
      const redirectUrl = productType === "predictions" ? "/predictions" : "/top-gainers"
    ...
    return NextResponse.redirect(`${origin}${redirectUrl}?${qs.toString()}`)
  } catch (error) {
    const origin = process.env.NEXT_PUBLIC_APP_ORIGIN || "https://hritik.vercel.app"
